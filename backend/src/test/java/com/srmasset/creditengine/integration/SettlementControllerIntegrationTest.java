...
class SettlementControllerIntegrationTest extends AbstractIntegrationTest {
    ...
    private Receivable newPendingReceivable(
            Currency faceCurrency, String documentNumber, LocalDate referenceDate) {
        Assignor assignor =
    ...
    void settlesInSameCurrencyWithNoFxConversion() throws Exception {
        ...
        SettlementRequest request =
                new SettlementRequest(
                        receivable.getId(), "BRL", new BigDecimal("2.0"), referenceDate);

        double countBefore =
                meterRegistry.get("settlements.count").tag("currency", "BRL").counter().count();
    ...
    void settlesWithInverseFxLookupWhenOnlyOppositeDirectionIsSeeded() throws Exception {
        ...
        SettlementRequest request =
                new SettlementRequest(
                        receivable.getId(), "USD", new BigDecimal("2.0"), referenceDate);
        ...
        BigDecimal expectedInverseRate =
                BigDecimal.ONE.divide(new BigDecimal("5.4"), 6, RoundingMode.HALF_EVEN);
    ...
    void batchProcessesEachItemIndependently() {
        ...
        settlementService.settle(
                receivable2.getId(), "BRL", new BaseRate(new BigDecimal("2.0"), referenceDate));

        var results =
                new com.srmasset.creditengine.domain.service.SettlementBatchService(
                                settlementService)
                        .settleBatch(
                                List.of(
                                        receivable1.getId(),
