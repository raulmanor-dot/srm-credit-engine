...
class ReceivableControllerIntegrationTest extends AbstractIntegrationTest {
	...
	void createsFindsUpdatesAndCancelsAReceivable() throws Exception {
		...
		ReceivableRequest createRequest = new ReceivableRequest(
				assignor.getId(), duplicata.getId(), brl.getId(), new BigDecimal("5000.00"), "DOC-CRUD-1", issueDate, dueDate);

		String body = mockMvc.perform(post("/receivables")
		...
		ReceivableUpdateRequest updateRequest = new ReceivableUpdateRequest(
				new BigDecimal("5500.00"), "DOC-CRUD-1-REV", issueDate, dueDate);
		mockMvc.perform(put("/receivables/{id}", id)
						.contentType(MediaType.APPLICATION_JSON)
		...
		mockMvc.perform(get("/receivables/{id}", id)).andExpect(status().isOk()).andExpect(jsonPath("$.status").value("CANCELED"));
	}
	...
	void rejectsUpdatingAndCancelingAlreadySettledReceivable() throws Exception {
		...
		ReceivableType duplicata = receivableTypeRepository.findByCode(ReceivableTypeCode.DUPLICATA_MERCANTIL)
				.orElseThrow();
		Assignor assignor = assignorRepository.save(new Assignor("Empresa CRUD Settled", "33444555000166"));

		LocalDate referenceDate = LocalDate.of(2026, 7, 7);
		ReceivableRequest createRequest = new ReceivableRequest(
				assignor.getId(), duplicata.getId(), brl.getId(), new BigDecimal("1000.00"), "DOC-CRUD-2", referenceDate,
		...
		SettlementRequest settlementRequest = new SettlementRequest(id, "BRL", new BigDecimal("2.0"), referenceDate);
		...
		ReceivableUpdateRequest updateRequest = new ReceivableUpdateRequest(
				new BigDecimal("2000.00"), "DOC-CRUD-2", referenceDate, referenceDate.plusDays(30));
		mockMvc.perform(put("/receivables/{id}", id)
						.contentType(MediaType.APPLICATION_JSON)
						.content(objectMapper.writeValueAsString(updateRequest)))
