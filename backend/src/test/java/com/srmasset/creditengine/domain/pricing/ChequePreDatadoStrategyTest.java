...
class ChequePreDatadoStrategyTest {
	...
	void discountsFaceValueByBaseRatePlusSpreadOverFractionalTerm() {
		ReceivableType type = new ReceivableType(
				ReceivableTypeCode.CHEQUE_PRE_DATADO, "Cheque Pre-datado", new BigDecimal("2.5"));
		...
		Receivable receivable = new Receivable(
				assignor, type, brl, new BigDecimal("5000.00"), "CHQ-1", referenceDate, dueDate);
		...
		BigDecimal expected = BigDecimal.valueOf(expectedDouble).setScale(2, RoundingMode.HALF_EVEN);
