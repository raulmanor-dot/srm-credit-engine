...
class DuplicataMercantilStrategyTest {
	...
	void discountsFaceValueByBaseRatePlusSpreadOverOneMonth() {
		ReceivableType type = new ReceivableType(
				ReceivableTypeCode.DUPLICATA_MERCANTIL, "Duplicata Mercantil", new BigDecimal("1.5"));
		...
		Receivable receivable = new Receivable(
				assignor, type, brl, new BigDecimal("10000.00"), "DOC-1", referenceDate, dueDate);
		...
		// PV = 10000 / 1,035
		BigDecimal expected = new BigDecimal("10000.00").divide(new BigDecimal("1.035"), 6, RoundingMode.HALF_EVEN);
	...
	void presentValueEqualsFaceValueWhenTermIsZero() {
		...
		LocalDate referenceDate = LocalDate.of(2026, 7, 7);
		Receivable receivable = new Receivable(
				assignor, type, brl, new BigDecimal("10000.00"), "DOC-1", referenceDate, referenceDate);

		BaseRate baseRate = new BaseRate(new BigDecimal("2.0"), referenceDate);

		BigDecimal presentValue = strategy.calculatePresentValue(receivable, baseRate);

		assertThat(presentValue.setScale(2, RoundingMode.HALF_EVEN)).isEqualByComparingTo(new BigDecimal("10000.00"));
	}
}
