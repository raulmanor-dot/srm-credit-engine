...
class SettlementMetricsRecorderTest {
	...
	void recordsCountAndVolumeTaggedByCurrency() {
		...
		assertThat(registry.get("settlements.count").tag("currency", "USD").counter().count()).isEqualTo(2.0);
		assertThat(registry.get("settlements.count").tag("currency", "BRL").counter().count()).isEqualTo(1.0);
		assertThat(registry.get("settlements.volume").tag("currency", "USD").summary().totalAmount())
				.isEqualTo(1500.50);
	}
}
