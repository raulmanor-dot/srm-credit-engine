...
/**
 * Registro tipo -> bean via injeção de {@code List<PricingStrategy>} pelo Spring (todo bean {@link
 * PricingStrategy} do contexto entra no mapa automaticamente). Adicionar um novo tipo de recebível
 * é criar uma nova classe {@code @Component} implementando {@link PricingStrategy}; este resolver
 * não precisa mudar — Open/Closed em vez de if/else por tipo.
 */
@Component
