...
/**
 * Base comum aos tipos de recebível precificados por juros compostos: taxa
 * mensal (base + spread do tipo) elevada ao prazo fracionário em meses. O
 * spread NÃO é constante Java — vem de
 * {@code receivable.getReceivableType().getSpreadPercentMonthly()}, ou seja,
 * é configurável em banco (tabela receivable_types) sem exigir deploy.
 *
 * <p>Um tipo futuro com fórmula distinta (ex.: juros simples) implementa
 * {@link PricingStrategy} diretamente, sem alterar esta classe nem o
 * {@link PricingStrategyResolver} — Open/Closed na prática.
 */
public abstract class AbstractCompoundInterestPricingStrategy implements PricingStrategy {
	...
	public BigDecimal calculatePresentValue(Receivable receivable, BaseRate baseRate) {
		...
		BigDecimal termInMonths = TermCalculator.monthsBetween(baseRate.referenceDate(), receivable.getDueDate(), MC);
