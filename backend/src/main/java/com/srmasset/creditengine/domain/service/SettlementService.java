...
/**
 * Liquida um único recebível: calcula o valor presente (moeda de face),
 * converte para a moeda de pagamento quando necessário e persiste o
 * {@link Settlement} como snapshot de auditoria de todas as taxas usadas.
 *
 * <p>{@code REQUIRES_NEW} (em vez de {@code REQUIRED}) torna explícito que
 * cada liquidação é uma unidade atômica independente — propriedade da qual
 * {@link SettlementBatchService} depende para processar um lote item a item
 * sem que a falha de um item afete os demais (ver ADR 0004).
 */
@Service
public class SettlementService {
	...
	public Settlement settle(Long receivableId, String paymentCurrencyCode, BaseRate baseRate) {
		...
		try {
			...
			BigDecimal presentValueFaceCurrency = strategy.calculatePresentValue(receivable, baseRate);
			long termDays = TermCalculator.daysBetween(baseRate.referenceDate(), receivable.getDueDate());
			BigDecimal termMonths = TermCalculator.monthsBetween(
			...
			Settlement settlement = new Settlement(
					...
					faceCurrency,
					baseRate.monthlyPercent().setScale(MathConstants.MONEY_SCALE, RoundingMode.HALF_EVEN),
					receivable.getReceivableType().getSpreadPercentMonthly().setScale(MathConstants.MONEY_SCALE, RoundingMode.HALF_EVEN),
					Math.toIntExact(termDays),
					termMonths.setScale(MathConstants.MONEY_SCALE, RoundingMode.HALF_EVEN),
					presentValueFaceCurrency.setScale(MathConstants.MONEY_SCALE, RoundingMode.HALF_EVEN),
					fxRateUsed == null ? null : fxRateUsed.setScale(MathConstants.MONEY_SCALE, RoundingMode.HALF_EVEN),
					netValuePaymentCurrency.setScale(MathConstants.MONEY_SCALE, RoundingMode.HALF_EVEN),
					OffsetDateTime.now());
			...
			settlementMetricsRecorder.recordSettlement(paymentCurrencyCode, netValuePaymentCurrency);

			log.info("Settlement {} concluída para o recebível {}: {} {} -> {} {}",
					settlement.getId(), receivableId,
					settlement.getFaceValue(), faceCurrency.getCode(),
					netValuePaymentCurrency.setScale(MathConstants.MONEY_SCALE, RoundingMode.HALF_EVEN), paymentCurrencyCode);
