...
/**
 * Gera uma cotação "fresca" para um par de moedas a partir de uma tabela de
 * taxas base configurada (ver {@code app.mock-provider.rates}), aplicando um
 * jitter aleatório para simular a variação natural de um provedor real.
 * Aceita também o par invertido (mesma lógica de {@code ExchangeRateService}),
 * já que só é prático configurar uma direção por par.
 */
@Component
public class MockRateGenerator {
	...
	public Optional<BigDecimal> generate(String baseCode, String quoteCode) {
		BigDecimal direct = properties.rates() == null ? null : properties.rates().get(key(baseCode, quoteCode));
		if (direct != null) {
			return Optional.of(applyJitter(direct));
		}
		BigDecimal inverse = properties.rates() == null ? null : properties.rates().get(key(quoteCode, baseCode));
		if (inverse != null) {
	...
	private BigDecimal applyJitter(BigDecimal baseRate) {
		...
		BigDecimal factor = BigDecimal.ONE.add(BigDecimal.valueOf(jitterFraction), MathConstants.PRICING_CONTEXT);
