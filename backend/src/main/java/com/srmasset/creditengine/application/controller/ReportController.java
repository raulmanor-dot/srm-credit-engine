...
/**
 * Exceção arquitetural deliberada: este controller fala direto com {@link
 * SettlementReportRepository} (SQL nativo), pulando {@code SettlementService}, a camada de domínio
 * e as entidades JPA — exatamente o caminho paralelo que o item 3.6 do enunciado permite para o
 * extrato de liquidação. É leitura pura para relatório, sem invariante de negócio a proteger, então
 * não há razão para passar pela camada de negócio. Ver README (seção Arquitetura) e ADR
 * correspondente.
 *
 * <p>Retorna {@link SettlementStatementRow} (tipo de {@code persistence.report}) diretamente na
 * resposta HTTP, em vez de remapear para um DTO de {@code application.dto} — isso é intencional:
 * remapear escondería o próprio ponto que este endpoint demonstra.
 */
@RestController
